...
* the benchmark queries (imported from ``query_examples.py`` so both files can
  never drift apart),
* query embedding via the project's ``OpenAIEmbedder``,
* latency statistics and the ``format_table`` pretty-printer (same style as
  ``compare_chunkers.py``).

Everything DB-side reuses the project's async session factory and
``search_chunks_by_cosine`` — no parallel engines, no raw clients.
"""
