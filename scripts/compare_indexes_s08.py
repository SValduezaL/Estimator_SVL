...
* ``ORDER BY embedding <=> :q`` → ``chunks_embedding_idx``
  (full-precision ``vector_cosine_ops``), via ``search_chunks_by_cosine``
  (the same path as ``/search``);
...
from app.foundation.persistence.database import get_session_factory  # noqa: E402
from app.foundation.persistence.models import Chunk, EMBEDDING_DIMENSIONS  # noqa: E402
from app.generation.rag.retrieval import search_chunks_by_cosine  # noqa: E402
...
async def assert_indexes_exist(session: AsyncSession) -> None:
    ...
    if missing:
        print(
            ...
            "Create them first: scripts/sql_S08/01_create_hnsw.sql (vector) and "
            "scripts/sql_S08/03_create_halfvec.sql (halfvec).",
            file=sys.stderr,
        )
        raise SystemExit(1)
...
def halfvec_statement(vector: list[float]):
    ...
    distance = cast(Chunk.embedding, HALFVEC(EMBEDDING_DIMENSIONS)).cosine_distance(vector)
    return select(Chunk.id).order_by(distance).limit(K)
    ...
    async with session_factory() as session:
        ...
        for _, query, vector in queries:
            ...
            async def via_vector_index() -> list[int]:
                results = await search_chunks_by_cosine(session, vector, k=K)
