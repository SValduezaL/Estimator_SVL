...
* Pre-flight (e.g. ``count=30000``): fatten the corpus so the Block 2
  ...
  the HNSW before/after gap is visible. With the real corpus from
  ``ingest_corpus.py`` a sequential scan is sub-millisecond and there is
  little to feel.
...
from app.foundation.persistence.models import Chunk, Document  # noqa: E402
from app.generation.rag.embedding.embedder import OpenAIEmbedder, estimate_embedding_cost_usd  # noqa: E402
from app.generation.rag.schemas import Chunk as ChunkSchema  # noqa: E402
...
async def find_or_create_synthetic_document(session) -> int:
    stmt = select(Document.id).where(Document.source_path == SYNTHETIC_SOURCE_PATH)
...
async def insert_synthetic(count: int) -> None:
    ...
    print(
        ...
        f"{total_tokens} tokens embedded, estimated cost ${estimate_embedding_cost_usd(total_tokens):.4f}"
    )
