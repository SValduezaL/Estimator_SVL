...
-- semánticas (p. ej. POST /search o scripts/query_examples.py), casi seguro
-- es el antipatrón operador/operator class del Bloque 2.3. Para verificar la
-- operator class de cada índice vectorial:
SELECT
    i.indexrelname,
...
--   · Los logs estructurados del servicio (structlog): correlacionad picos
--     de latencia en POST /search con los planes de EXPLAIN de aquí.
