...
stats and/or the top-k chunks each strategy retrieves per query.

This is the tool used during the Session 7 demos. Nothing is persisted;
...
Examples::
    ...
    uv run python scripts/compare_chunkers.py --strategies all --queries all \\
        --show-stats --show-cost --output COMPARISON_REPORT.md
"""
...
def build_embedder() -> OpenAIEmbedder:
    ...
    return OpenAIEmbedder(model=settings.semantic_embedding_model)
...
def write_report(path: Path, stats: dict, queries: dict) -> None:
    lines = [
        ...
        "",
        "> Generado por `scripts/compare_chunkers.py --output ...`.",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--models", default=None, help="e.g. small-1536")
    ...
    settings = get_settings()
    try:
        chunkers = build_chunkers(strategies, settings=settings)
    except KeyError as exc:
        raise SystemExit(f"Unknown strategy: {exc.args[0]}. Available: {ALL_STRATEGIES}") from exc
